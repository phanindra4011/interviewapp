...
export async function aiInterviewer(input: AIInterviewerInput): Promise<AIInterviewerOutput> {
  ...
  const userSeemsUnsure = /\b(i\s*don'?t\s*know|dont\s*know|not\s*sure|no\s*idea|idk|skip\s*this)\b/i.test(input.userResponse);
  const extraGuidance = userSeemsUnsure
    ? `Since the user's response indicates uncertainty or that they don't know, include in the feedback a concise, correct answer labeled exactly as: "Correct answer: <answer>". Keep it 1-2 sentences.`
